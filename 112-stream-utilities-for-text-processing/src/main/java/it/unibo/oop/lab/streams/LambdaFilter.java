...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        LOWERCASE("Set to lowercase", String::toLowerCase),
        CHARCOUNT("Char count", s -> Integer.toString(s.length())),
        LINECOUNT("Line count", s -> Long.toString(s.chars().filter(e -> e == '\n').count() + 1)),
        ORDEREDWORDS("Alphabetical order", s ->
            Arrays.stream(s.split(ANY_NON_WORD))
                    .sorted()
                    .collect(Collectors.joining("\n"))),
        WORDSCOUNT("Count every instance of a word", s ->
            Arrays.stream(s.split(ANY_NON_WORD))
